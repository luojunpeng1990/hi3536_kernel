...
static void __init hi3536_clocksource_init(void __iomem *base,
						const char *name)
{
...
void __init hi3536_timer_init(void)
{
	...
	writel(readl((void *)IO_ADDRESS(REG_BASE_SCTL)) |
		(1 << 20) | (1 << 22),
		(void *)IO_ADDRESS(REG_BASE_SCTL));
#ifdef CONFIG_ARM_LOCAL_TIMER
	...
	hi3536_clocksource_init((void *)TIMER(2)->addr,
		TIMER(2)->name);
