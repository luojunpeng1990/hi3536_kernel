...
#ifdef CONFIG_TNK
	/* when shutdown(WR), TOE connection will be removed and recv packet
	 * will be moved to bypass recv queue. If then there are more packets
	 * coming, tcp_try_coalesce will merge skb and change sk_rmem_alloc.
	 * If this happen, sk_rmem_alloc and sk_forward_alloc will be none zero
	 * and WARNING will be print in following code:
	 * ---net/core/stream.c:201 inet_csk_destroy_sock+0x60/0x154()
	 * ---net/ipv4/af_inet.c:166 inet_sock_destruct+0x194/0x1d4()
	 * So if the packet is from TOE, return false to disable skb merge.
	 */
...
int tnk_tcp_fin_acked_state_process(struct sock *sk)
{
	...
	case TCP_FIN_WAIT1:
		...
		if (!sock_flag(sk, SOCK_DEAD))
			...
			/* Risk exist when we create TIME_WAIT socket,
			 * because TIME_WAIT socket is not locked,
			 * packet will go to TIME_WATI socket by other CPU
			 */
#if 0
			tmo = tcp_fin_time(sk);
			if (tmo > TCP_TIMEWAIT_LEN) {
